...
|------|------|
| Python 文件 | 410 个（含测试） |
| Python 总行数 | 119,310 行（含测试） |
| 测试文件 | 162 个 .py |
| 测试用例 | 2544 个离线收集项（完整质量门通过） |
| 文档 | 8 个（精简后） |
| 注册统计方法 | 48 个 |
| 规范方法 ID | 35 个 |
| APA 表格路由组 | 21 个 |
...
| v5.8 | 启动与大文件导入性能优化、环境检查降级与关键兼容修复 |
| v5.9 | 交互性能、图表故障隔离、常用统计图修复与危险操作确认 |
| **v5.10** | **统一主导航并接通实验设计；安全开始新研究；恢复新手引导；下一步导航；原子自动保存与隐私/版本一致性** |
