...
TEST_TEMPLATES: Dict[str, Dict[str, Any]] = {
    ...
    "paired_ttest": {
        "why": "配对样本t检验适用于同一组被试在两种条件下（如前后测）的比较。因为数据存在自然配对，使用独立样本t检验会损失统计效力。",
        "steps": [
    ...
    "two_way_anova": {
        "why": "双因素方差分析用于同时考察两个自变量的主效应及其交互作用，提高统计效力并更贴近真实情境。",
        "steps": [
            "计算两个自变量的主效应",
            "计算交互效应（A×B）",
            "交互效应显著 → 需做简单效应分析",
    ...
    "ancova": {
        "why": "协方差分析（ANCOVA）在方差分析的基础上纳入连续协变量，在统计上控制协变量的影响后比较组间差异，提高统计效力。",
        "steps": [
            "检验回归斜率同质性假设",
